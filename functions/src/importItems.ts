...
export default async function (
  ...
  try {
    ...
    context.info({ triggerMetadata: context.triggerMetadata });
    const courseName: string = context.triggerMetadata?.courseName as string;
    const testName: string = context.triggerMetadata?.testName as string;
    context.info({ courseName, testName });

    // Blobトリガーで受け取ったjsonファイルのバイナリデータをImportItem[]型として読込み
    context.info({ blobType: typeof blob });
    ...
    const insertedImportItems: ImportItem[] = insertedQuestionItems.map(
      (insertedQuestionItem: Question) => {
        ...
        if (insertedQuestionItem.explanations) {
          insertedImportItem.explanations = insertedQuestionItem.explanations;
        }
